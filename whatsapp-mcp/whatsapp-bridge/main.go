...
func startRESTServer(client *whatsmeow.Client, messageStore *MessageStore, cfg *config.Config) {
	// Create file validator from config
	validator := validation.NewValidator(cfg.Files.MaxSizeMB, cfg.Files.AllowedTypes)

	// Create API handler
	handler := api.NewHandler(client, messageStore, validator)
	...
	fmt.Printf("  POST   /api/send-message\n")
	fmt.Printf("  POST   /api/send-file\n")
	fmt.Printf("  POST   /api/send-file-with-message\n")
	fmt.Printf("  GET    /api/health\n")
	fmt.Printf("  GET    /api/status\n")
	fmt.Printf("  POST   /api/send (legacy - still works)\n")
	fmt.Printf("  POST   /api/download (legacy - still works)\n")
...
func main() {
	...
	if client.Store.ID == nil {
		...
		case <-time.After(time.Duration(cfg.WhatsApp.TimeoutSeconds) * time.Second):
